...
def point_cloud_generator(worker_id, queue):
    # Generate a single point cloud and notify the master node
    points, colors = process_pointcloud(worker_id)  # Generate the point cloud
    queue.put({"worker_id": worker_id, "points": points.tolist(), "colors": colors.tolist()})  # Notify master node
    print(f"Worker {worker_id} generated a point cloud and notified the master node.")

# Master node logic to handle worker notifications and inform the UI
async def master_node_handler(viser_server, worker_queues):
    while True:
        try:
            for queue in worker_queues:
                if not queue.empty():
                    # Get the point cloud data from the worker
                    data = queue.get()
                    worker_id = data["worker_id"]
                    points = np.array(data["points"])
                    colors = np.array(data["colors"])

                    # Notify the UI and add the point cloud to the graph
                    print(f"Master node received point cloud from worker {worker_id}. Adding to UI...")
                    viser_server.scene.add_point_cloud(
                        name=f"worker_{worker_id}",
                        points=points,
                        colors=colors,
                    )
                    print(f"Point cloud from worker {worker_id} added to the UI.")
            
            # Wait before checking the queues again
            await asyncio.sleep(1)
        except Exception as e:
            print(f"Error in master node handler: {e}")
...
if __name__ == "__main__":
    ...
    # Start point cloud generators for each worker
    point_cloud_queues = [Queue() for _ in range(args.nodes)]
    generator_processes = [
        Process(target=point_cloud_generator, args=(worker_id, point_cloud_queues[worker_id]))
        for worker_id in range(args.nodes)
    ]
    for process in generator_processes:
        process.start()

    # Start the master node handler to process worker notifications
    asyncio.run(master_node_handler(main_viser_server, point_cloud_queues))
